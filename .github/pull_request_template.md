...
- **CodeRabbit** (if enabled on the repo): Expect AI review comments on style and consistency. Treat **`.cursorrules`** as the contract for this project: mandatory `element_label` on `BasePage` interactions, resilient `.or_()` on critical locators, `expect()` for UI state, no `page.wait_for_timeout()`.
...
- [ ] No new `wait_for_timeout` calls; UI waits use Playwright auto-wait + `expect` as per `.cursorrules`.
