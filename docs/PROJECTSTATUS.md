...
| Phase | Status | Focus |
|-------|--------|--------|
| **1** | **Done** | **Infrastructure** — `Dockerfile` + CI **smoke** on `fatf-smoke` (Buildx/GHA cache, `docker cp` → `smoke-report`); non-root `fatf`; `.env.smoke`. Full **test** job remains host Python. |
...
| **CI** (`../.github/workflows/test.yml`) | **smoke** builds/runs `fatf-smoke` (artifact `smoke-report`); **test** is host Python + pytest-xdist; separate AI failure analysis workflow ([ai-failure-analysis.yml](../.github/workflows/ai-failure-analysis.yml)) |
